...
class HeaderBar(unittest.TestCase):
    ...
    def test_click_create(self):
        '''测试点击创作跳转至GUI编辑器'''
        tab = self.driver.find_element_by_xpath('//*[@id="app"]/div/div/div[1]/div[1]/div/button')
        tab.click()
        time.sleep(2)
        except_url = "https://gui.bellcode.com/index"
        now_handle = self.driver.current_window_handle #获取当前窗口句柄
        all_handles = self.driver.window_handles #获取所有窗口句柄
        for handle in all_handles:
            if handle != now_handle:
    ...
    def test_click_login(self):
        '''测试点击登录跳转至登录页面'''
        tab = self.driver.find_element_by_link_text("注册/登录")
        tab.click()
        time.sleep(2)
        cur_url = self.driver.current_url
        except_url = self.base_url + "/#/login"
